...
return (
    ...
    {data.places.data.map((place)=>{
        return (
          <div className='mx-12 pt-8 mt-4  relative ' key={place.id}  >
            <Link
            to={`/productDetails/${place.id}`}
            state={{place}}
             >
            <div  className='w-2/4 h-20 rounded-md flex flex-col hover:bg-gradient-to-r border-2 border-slate-700  cursor-pointer shadow-lg transition ease-in-out delay-50  hover:-translate-y-1 hover:scale-110 hover:bg-indigo-500 duration-300'>
             <div className='w-full h-3/5 flex  border-slate-700'>
           ...
           <div className='w-full h-2/5  flex flex-row items-center ml-5 border-slate-700 '>
               <h4 className='relative  left-10 text-xl '><span className='text-2xl font-normal mr-2'>{place.attributes.seat}</span>Seats left</h4>
               <h4 className='relative  left-52 text-xl'>Rs- <span className='text-2xl font-normal mr-3'>{place.attributes.cost}/</span>Only</h4>
           </div>
            
         </div>
         <div className='absolute w-10 h-10 bg-teal-200 top-3 flex justify-center items-center rounded-lg -left-4'>
           <p className='text-2xl font-bold'>{getNumberOfDays(place.attributes.startDate,place.attributes.endDate)}</p>
         </div>
         </Link>
        </div>
        )
